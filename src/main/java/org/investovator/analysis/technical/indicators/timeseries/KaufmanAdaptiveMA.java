 ...
 * @version ${Revision}
 */
public class KaufmanAdaptiveMA extends TimeSeriesIndicator {
    ...
    public ResultsSet calculate(Params parameters) throws InvalidParamException, AnalysisException {
        ...
        if(isParametersValid(timeSeriesParams)){
            try {
                ...
                Core core = new Core();
                RetCode retCode = core.kama(0, (closingPrices.length - 1), closingPrices, timeSeriesParams.getPeriod(),
                        begin, length, out);
                ...
                if(retCode == RetCode.Success){
                    ...
                    resultSet.setGraph(TimeSeriesGraph.KAMA, dates, out, begin.value, length.value);
                    return resultSet;
                } else {
