 ...
 * @version ${Revision}
 */
public class ChandeMomentumOscillator extends TimeSeriesIndicator {
    ...
    public ResultsSet calculate(Params parameters) throws InvalidParamException, AnalysisException {
        ...
        if(isParametersValid(timeSeriesParams)){
            try {
                ...
                Core core = new Core();
                RetCode retCode = core.cmo(0, (closingPrices.length - 1), closingPrices, timeSeriesParams.getPeriod(),
                        begin, length, out);
                ...
                if(retCode == RetCode.Success){
                    ...
                    resultSet.setGraph(TimeSeriesGraph.CMO, dates, out, begin.value, length.value);
                    return resultSet;
                } else {
