 ...
 * @version ${Revision}
 */
public class TripleExponentialMA extends TimeSeriesIndicator {
    ...
    public ResultsSet calculate(Params parameters) throws InvalidParamException, AnalysisException {
        ...
        if(isParametersValid(timeSeriesParams)){
            try {
                ...
                Core core = new Core();
                RetCode retCode = core.tema(0, (closingPrices.length - 1), closingPrices, timeSeriesParams.getPeriod(),
                        begin, length, out);
                ...
                if(retCode == RetCode.Success){
                    ...
                    resultSet.setGraph(TimeSeriesGraph.TEMA, dates, out, begin.value, length.value);
                    return resultSet;
                } else {
