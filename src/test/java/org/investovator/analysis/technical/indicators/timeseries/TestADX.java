 ...
 * @version ${Revision}
 */
public class TestADX extends TestIndicator {

    @Test
    public void testADXCalculate() throws AnalysisException, ParseException, InvalidParamException {
        ...
        TimeSeriesResultSet resultSet = (TimeSeriesResultSet) calculator.calculateValues(IndicatorType.ADX, params);
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.ORIGINAL));
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.ADX));

        String randomDate = "2/15/2010";
        assertEquals((resultSet.getGraph(TimeSeriesGraph.ADX)).get(format.parse(randomDate)), 33.90139570081902);
    }
}
