 ...
 * @version ${Revision}
 */
public class TestTRIMA extends TestIndicator {

    @Test
    public void testTRIMACalculate() throws AnalysisException, ParseException, InvalidParamException {
        ...
        TimeSeriesResultSet resultSet = (TimeSeriesResultSet) calculator.calculateValues(IndicatorType.TRIMA, params);
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.ORIGINAL_CLOSING));
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.TRIMA));

        String randomDate = "2/15/2010";
        assertEquals((resultSet.getGraph(TimeSeriesGraph.TRIMA)).get(format.parse(randomDate)), 220.234375);
    }
}
