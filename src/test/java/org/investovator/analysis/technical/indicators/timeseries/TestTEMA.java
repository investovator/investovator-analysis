 ...
 * @version ${Revision}
 */
public class TestTEMA extends TestIndicator {

    @Test
    public void testTEMACalculate() throws AnalysisException, ParseException, InvalidParamException {
        ...
        TimeSeriesResultSet resultSet = (TimeSeriesResultSet) calculator.calculateValues(IndicatorType.TEMA, params);
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.ORIGINAL_CLOSING));
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.TEMA));

        String randomDate = "2/15/2010";
        assertEquals((resultSet.getGraph(TimeSeriesGraph.TEMA)).get(format.parse(randomDate)), 213.2475810815756);
    }
}
