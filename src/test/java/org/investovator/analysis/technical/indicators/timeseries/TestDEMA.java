 ...
 * @version ${Revision}
 */
public class TestDEMA extends TestIndicator {

    @Test
    public void testDEMACalculate() throws AnalysisException, ParseException, InvalidParamException {
        ...
        TimeSeriesResultSet resultSet = (TimeSeriesResultSet) calculator.calculateValues(IndicatorType.DEMA, params);
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.ORIGINAL_CLOSING));
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.DEMA));

        String randomDate = "2/15/2010";
        assertEquals((resultSet.getGraph(TimeSeriesGraph.DEMA)).get(format.parse(randomDate)), 215.5028521919502);
    }
}
