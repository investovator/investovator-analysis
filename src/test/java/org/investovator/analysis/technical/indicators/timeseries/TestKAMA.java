 ...
 * @version ${Revision}
 */
public class TestKAMA extends TestIndicator {

    @Test
    public void testKAMACalculate() throws AnalysisException, ParseException, InvalidParamException {
        ...
        TimeSeriesResultSet resultSet = (TimeSeriesResultSet) calculator.calculateValues(IndicatorType.KAMA, params);
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.ORIGINAL));
        assertTrue(resultSet.containsGraph(TimeSeriesGraph.KAMA));

        String randomDate = "2/15/2010";
        assertEquals((resultSet.getGraph(TimeSeriesGraph.KAMA)).get(format.parse(randomDate)), 218.5236064713244);
    }
}
